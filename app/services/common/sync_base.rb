class Common::SyncBase
  ...
  def rpc_get( path, params=nil )
    ...
    body = Rails.cache.fetch( ['rpc_get', @chain.network_name.downcase, @chain.ext_id.to_s, path].join('-'), force: Rails.env.development?, expires_in: 30.seconds, version: CACHE_VERSION ) do
  ...
  def lcd_get( path, params=nil )
    ...
    body = Rails.cache.fetch( ['lcd_get', @chain.network_name.downcase, @chain.ext_id.to_s, path].join('-'), force: Rails.env.development?, expires_in: 30.seconds, version: CACHE_VERSION ) do
      start_time = Time.now.utc.to_f
  ...
  def lcd_post( path, body )
    ...
    body = Rails.cache.fetch( ['lcd_post', @chain.network_name.downcase, @chain.ext_id.to_s, path].join('-'), force: Rails.env.development?, expires_in: 30.seconds, version: CACHE_VERSION ) do
      start_time = Time.now.utc.to_f
