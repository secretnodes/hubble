class Cosmos::AverageSnapshotsGeneratorService
  ...
  def hourly_average_snapshots!( kind, object_scope=nil, &value_generator )
    ...
    if !latest
      ...
      latest = adjust_for_interval('hour', latest + 1.public_send('hour'), :beginning)
    end
    ...
    target = [ 1.public_send('hour').ago, @chain.blocks.first.try(:timestamp) - 1.public_send('hour') ].min
  ...
  def daily_average_snapshots!( kind, object_scope=nil )
    ...
    if !latest
      ...
      # we push 1 more {interval} since we already have the snapshot for 'latest'
      latest = adjust_for_interval('day', latest + 1.public_send('day'), :beginning)
    end
    ...
    # now just keep going through the {interval}s until we reach the last time period
    target = [ 1.public_send('day').ago, @chain.blocks.first.timestamp - 1.day ].min
