class Cosmos::ValidatorEventsService
      ...
      insert_events( from, to ) do |block|
        existing_validators.values.each do |validator|
          ...
          if tripped
            if !latch.held?
              begin
                Cosmos::ValidatorEvents::NConsecutive.create!(
                  chain_id: @chain.id,
                ...
                raise Cosmos::SyncBase::CriticalError.new("Could not create n-consecutive validator event. #{validator.address} #{n} at height #{block.height}")
              end
      ...
      insert_events( from, to ) do |block|
        existing_validators.values.each do |validator|
          ...
          if tripped
            if !latch.held?
              begin
                Cosmos::ValidatorEvents::NOfM.create!(
                  chain_id: @chain.id,
                  validator_id: validator.id,
                  height: block.height,
                  timestamp: block.timestamp,
                ...
                raise Cosmos::SyncBase::CriticalError.new("Could not create n-of-m validator event. #{validator.address} #{n} of #{m} at height #{block.height}")
              end
  ...
  def run_voting_power_change!( defn_id )
    ...
    if to < from
      ...
      latest_voting_power_changes = existing_validators.values.each_with_object({}) do |v, h|
        h[v.address] = v.events.voting_power_change.first.try(:to)
      end
      ...
      insert_events( from, to, get_active_set_changes ) do |block, active_set_changes|
        ...
        block.validator_set.each do |address, voting_power|
          ...
          if !existing_validators.keys.include?( address )
            raise Cosmos::SyncBase::CriticalError.new("Unknown validator #{address} in validator set for block #{block.height}.")
          end
          ...
          if latest_voting_power_changes[address] != voting_power
            if !active_set_change || active_set_change.added?
              ...
              if latest_voting_power_changes[address].blank? ||
                 Cosmos::ValidatorEvents::VotingPowerChange.significant_change?(
                   latest_voting_power_changes[address],
                   voting_power
                 )
                begin
                  Cosmos::ValidatorEvents::VotingPowerChange.create!(
                    chain_id: @chain.id,
                    validator_id: validator.id,
                    height: block.height,
                    timestamp: block.timestamp,
                  ...
                  raise Cosmos::SyncBase::CriticalError.new("Could not create voting-power-change validator event. #{validator.address} #{voting_power} at height #{block.height}")
                end
              end
        ...
        validators.keys.each do |address|
          ...
          begin
            if removed
              ...
              Cosmos::ValidatorEvents::ActiveSetInclusion.create!(
                chain_id: @chain.id,
            ...
            raise Cosmos::SyncBase::CriticalError.new("Could not create active-set-inclusion validator event. #{validator.address} #{added ? 'added' : removed ? 'removed' : 'no change...?'} at height #{block.height}")
          end
        end
  ...
  def insert_events( from, to, extra_arg_proc=nil, &process )
